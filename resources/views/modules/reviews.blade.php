		...
		<div class="modal fade" id="modalForm" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalFormLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-scrollable">
				...
				<div class="modal-body">
						...
						<div class="mb-3">
							<label for="name" class="col-form-label">Nama Reviewer:</label>
						...
						<div class="form-check mb-2">
							<input class="form-check-input" type="radio" name="verify" id="verify1" value="1">
							<label class="form-check-label" for="verify1">
							  Disetujui
							</label>
						</div>
						<div class="form-check">
							<input class="form-check-input" type="radio" name="verify" id="verify2" value="0" checked>
    ...
    <script>
        ...
        $(document).ready(function() {
			oTable = $('#datatable_sipayu').DataTable( {
				'ajax': {
					'url': `${baseUrlApi}/api/review/list_dt`,
					'type': 'GET',
				...
				"columns": [
					...
					{ 
						...
						"render": function(row) {
							let html = `
								${row.verify ? 'Disetujui' : 'Belum disetujui'}
							`;
							return html;
						}
					},
					...
					{ 
						...
						"render": function(row) {
							let html = `
								<a href="javascript:void(0)" class="btn btn-sm btn-info edit" data-id="${row.id_review}">Ubah</a>
								<a href="javascript:void(0)" class="btn btn-sm btn-danger delete" data-id="${row.id_review}">Hapus</a>
							`;
							return html;
						}
					},
		...
		async function deleteData(id) {
			...
			let execute = async (id) => {
				let config = {
				  method: 'delete',
				  maxBodyLength: Infinity,
				  url: `${baseUrlApi}/api/review/delete/${id}`,
				  headers: { 
					"Authorization": `Bearer ${apiKey}` 
				  }
				};
		...
		async function submitData() {
			...
			let isUrl = action == "add" ? `${baseUrlApi}/api/review/create` : `${baseUrlApi}/api/review/update/${$("#id").val()}`;
			let config = {
			  method: 'post',
			  maxBodyLength: Infinity,
			  url: isUrl,
