		...
		<div class="modal fade" id="modalForm" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalFormLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-scrollable">
				...
				<div class="modal-body">
						...
						<div class="mb-3">
							<label for="date_event" class="col-form-label">Tanggal Event:</label>
							<input type="datetime-local" name="name" class="form-control" id="date_event" required />
						</div>
						<div class="mb-3">
							<label for="description" class="col-form-label">Description/Kontak:</label>
							<textarea class="form-control" name="description" id="description" required></textarea>
							<label for="date_event"><small>Anda dapat mengisi penjelasan event dan atau kontak</small></label>
						</div>
						<div class="mb-3">
    ...
    <script>
        ...
        $(document).ready(function() {
			oTable = $('#datatable_sipayu').DataTable( {
				'ajax': {
					'url': `${baseUrlApi}/api/event/list_dt`,
					'type': 'GET',
		...
		async function deleteData(id) {
			...
			let execute = async (id) => {
				let config = {
				  method: 'delete',
				  maxBodyLength: Infinity,
				  url: `${baseUrlApi}/api/event/delete/${id}`,
				  headers: { 
					"Authorization": `Bearer ${apiKey}` 
				  }
				};
		...
		async function submitData() {
			if ($("#imageBase64").val()!='') {
				...
				let isUrl = action == "add" ? `${baseUrlApi}/api/event/create` : `${baseUrlApi}/api/event/update/${$("#id").val()}`;
				let config = {
				  method: 'post',
				  maxBodyLength: Infinity,
				  url: isUrl,
